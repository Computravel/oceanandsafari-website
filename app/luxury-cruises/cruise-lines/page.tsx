...
export const metadata: Metadata = {
  title: "Cruise Lines | Ocean & Safari",
  description: "Discover our curated collection of luxury cruise lines, from ultra-luxury ocean voyages to expedition and river cruising.",
};
  ...
  return (
    <main style={{ fontFamily: "var(--font-jost), sans-serif", background: "var(--pearl)" }}>
      ...
      <section style={{ background: "var(--abyss)", padding: "100px 40px 80px", textAlign: "center" }}>
        <div style={{ fontFamily: "var(--font-jost), sans-serif", fontSize: "11px", letterSpacing: "0.24em", textTransform: "uppercase", color: "var(--teal)", fontWeight: 500, marginBottom: "16px" }}>Set Sail in Style</div>
        <h1 style={{ fontFamily: "var(--font-cormorant), serif", fontSize: "clamp(40px, 6vw, 68px)", fontWeight: 400, color: "var(--pearl)", lineHeight: 1.2, marginBottom: "20px" }}>Cruise Lines</h1>
        <p style={{ fontFamily: "var(--font-jost), sans-serif", fontSize: "17px", color: "rgba(247,242,234,0.7)", lineHeight: 1.8, maxWidth: "560px", margin: "0 auto" }}>
          A curated collection of luxury, premium and expedition cruise lines, each with their own character and way of taking you to sea.
        </p>
      </section>
        ...
        {cruiseLines && cruiseLines.length > 0 ? (
          <div className="packages-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "24px" }}>
            {cruiseLines.map((cruiseLine: any) => (
              <Link key={cruiseLine._id} href={`/luxury-cruises/cruise-lines/${cruiseLine.slug?.current}`} style={{ textDecoration: "none" }}>
                  ...
                  <div style={{ height: "160px", overflow: "hidden", background: "var(--abyss)", position: "relative", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    ...
                    {cruiseLine.category && (
                      <div style={{ position: "absolute", top: "12px", left: "12px", fontFamily: "var(--font-jost), sans-serif", fontSize: "11px", letterSpacing: "0.14em", textTransform: "uppercase", fontWeight: 500, background: "rgba(11,31,58,0.75)", color: "white", padding: "4px 10px", borderRadius: "2px", backdropFilter: "blur(4px)" }}>
                        {CATEGORY_LABELS[cruiseLine.category] || cruiseLine.category}
                      </div>
                    )}
                  </div>
                  <div style={{ padding: "20px", display: "flex", flexDirection: "column", flex: 1 }}>
                    <div style={{ fontFamily: "var(--font-cormorant), serif", fontSize: "22px", color: "var(--charcoal)", marginBottom: "6px", lineHeight: 1.3 }}>{cruiseLine.name}</div>
          ...
          <div style={{ textAlign: "center", padding: "80px 40px", fontFamily: "var(--font-cormorant), serif", fontSize: "24px", color: "var(--muted)" }}>
            Cruise lines coming soon — contact us to plan your bespoke voyage.
          </div>
        )}
      </section>
